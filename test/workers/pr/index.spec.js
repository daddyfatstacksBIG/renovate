...
describe('workers/pr', () => {
  describe('checkAutoMerge(pr, config)', () => {
    ...
    it('should not automerge if enabled and pr is mergeable but cannot rebase',
       async () => {
         config.automerge = true;
         pr.isModified = true;
         platform.getBranchStatus.mockReturnValueOnce('success');
    ...
    it('should not automerge if enabled and pr is mergeable but branch status is not success',
       async () => {
         config.automerge = true;
    ...
    it('should not automerge if enabled and pr is mergeable but unstable',
       async () => {
         config.automerge = true;
  ...
  describe('ensurePr', () => {
    ...
    it('should return needs-approval if prCreation set to approval',
       async () => {
         platform.getBranchStatus.mockReturnValueOnce('success');
    ...
    it('should add note about Pin', async () => {
      ...
      expect(pr).toMatchObject({displayNumber : 'New Pull Request'});
      expect(platform.createPr.mock.calls[0]).toMatchSnapshot();
      expect(platform.createPr.mock.calls[0][2].includes('this Pin PR'))
          .toBe(true);
    });
    ...
    it('should not add assignees and reviewers to new PR if automerging enabled',
       async () => {
    ...
    it('should add assignees and reviewers to new PR if automerging enabled but configured to always assign',
       async () => {
         config.assignees = [ 'bar' ];
         config.reviewers = [ 'baz' ];
         config.automerge = true;
    ...
    it('should add random sample of assignees and reviewers to new PR',
       async () => {
    ...
    it('should return unmodified existing PR if only whitespace changes',
       async () => {
         const modifiedPr = JSON.parse(JSON.stringify(existingPr)
                                           .replace(' ', '  ')
                                           .replace('\n', '\r\n'));
    ...
    it('should not return null if branch automerging taking too long',
       async () => {
         config.automerge = true;
         config.automergeType = 'branch';
         platform.getBranchStatus.mockReturnValueOnce('pending');
    ...
    it('should create PR if waiting for not pending but artifactErrors',
       async () => {
         platform.getBranchStatus.mockReturnValueOnce('pending');
         platform.getBranchLastCommitTime.mockImplementationOnce(
             () => new Date());
