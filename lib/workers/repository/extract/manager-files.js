...
async function getManagerPackageFiles(config, managerConfig) {
  ...
  const matchedFiles =
      getMatchingFiles(fileList, manager, managerConfig.fileMatch);
  // istanbul ignore else
  if (matchedFiles.length) {
    logger.debug(`Matched ${matchedFiles.length} file(s) for manager ${
        manager}: ${matchedFiles.join(', ')}`);
  } else {
    return [];
  }
  ...
  for (const packageFile of matchedFiles) {
    ...
    if (content) {
      const res =
          await extractPackageFile(manager, content, packageFile, config);
      if (res) {
        packageFiles.push({
          packageFile,
          manager,
