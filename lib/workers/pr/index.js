...
async function ensurePr(prConfig) {
  ...
  async function getBranchStatus() {
    if (!branchStatus) {
      branchStatus = await platform.getBranchStatus(
          branchName, config.requiredStatusChecks);
  ...
  if (config.automerge === true && config.automergeType.startsWith('branch') &&
    ...
    if ((await getBranchStatus()) === 'pending' ||
      ...
      const elapsedHours =
          Math.round((currentTime.getTime() - lastCommitTime.getTime()) /
                     millisecondsPerHour);
  ...
  if (config.prCreation === 'status-success') {
  ...
  } else if (config.prCreation === 'approval' && !existingPr &&
    ...
    if ((await getBranchStatus()) === 'pending' ||
      ...
      logger.debug(`prNotPendingHours=${
          config.prNotPendingHours} threshold hit - creating PR`);
    }
    logger.debug('Branch status success');
  }
  ...
  for (const upgrade of upgrades) {
    const upgradeKey =
        `${upgrade.depType}-${upgrade.depName}-${upgrade.manager}-${
    ...
    if (logJSON) {
      ...
      upgrade.releases = [];
      if (upgrade.hasReleaseNotes && upgrade.githubName &&
  ...
  try {
    if (existingPr) {
      ...
      const reviewableIndex =
          existingPr.body.indexOf('<!-- Reviewable:start -->');
...
async function addAssigneesReviewers(config, pr) {
  if (config.assignees.length > 0) {
    try {
      let assignees = config.assignees.map(
          assignee => assignee.length && assignee[0] === '@' ? assignee.slice(1)
                                                             : assignee);
      ...
      logger.info({assignees : config.assignees, err},
                  'Failed to add assignees');
    }
  }
  if (config.reviewers.length > 0) {
    try {
      let reviewers = config.reviewers.map(
          reviewer => reviewer.length && reviewer[0] === '@' ? reviewer.slice(1)
                                                             : reviewer);
      ...
      logger.info({reviewers : config.reviewers, err},
...
async function checkAutoMerge(pr, config) {
  ...
  if (automerge) {
    ...
    const branchStatus =
        await platform.getBranchStatus(branchName, requiredStatusChecks);
    ...
    if (automergeType === 'pr-comment') {
      ...
      // istanbul ignore if
      if (config.dryRun) {
        logger.info('DRY-RUN: Would add PR automerge comment to PR #' +
                    pr.number);
        return false;
      }
...
function noWhitespace(input) { return input.replace(/\r?\n|\r|\s/g, ''); }
